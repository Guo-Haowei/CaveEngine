...
void PickingService::Register(IPickConsumer* p_consumer) {
    ...
    LOG_TRACE("PickingService::Register: register picking consumer '{}(id:{})'", id.type, id.uid);
#endif
}
...
#if USING(USE_LOG)
    DebugId id = p_consumer->GetDebugId();
    LOG_TRACE("PickingService::Unegister: unregister picking consumer '{}(id:{})'", id.type, id.uid);
#endif
}
