...
void ImageCanvas::setZoom(float zoom,
                          float min_zoom,
                          float max_zoom) {
    m_zoom = math::clamp(zoom, min_zoom, max_zoom);
...
void ImageCanvas::applyFit(const ImageCanvasDesc& desc,
    ...
    m_zoom = math::clamp(math::min(zoom_x, zoom_y), desc.min_zoom, desc.max_zoom);

    m_fit_requested = false;
}

void ImageCanvas::applyMouseWheelZoom(const ImageCanvasDesc& desc,
                                      const ImVec2& image_min_ss,
                                      const ImVec2& displayed_size) {
    unused(displayed_size);

    if (!desc.allow_mouse_wheel_zoom || !ImGui::IsWindowHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem)) {
        return;
    }
    ...
    const float factor = wheel > 0.0f
                             ? std::pow(desc.zoom_step, wheel)
                             : std::pow(desc.zoom_step, wheel);

    const float new_zoom = math::clamp(old_zoom * factor, desc.min_zoom, desc.max_zoom);

    if (math::abs(new_zoom - old_zoom) < 0.000001f) {
        return;
    }

    // Preserve the texel under the cursor by adjusting the child scroll.
    const float mouse_image_x = (mouse_ss.x - image_min_ss.x) / old_zoom;
    const float mouse_image_y = (mouse_ss.y - image_min_ss.y) / old_zoom;
    const float new_display_x = mouse_image_x * new_zoom;
    const float new_display_y = mouse_image_y * new_zoom;
    const float old_display_x = mouse_image_x * old_zoom;
    const float old_display_y = mouse_image_y * old_zoom;

    const float scroll_x = ImGui::GetScrollX() + (new_display_x - old_display_x);
    const float scroll_y = ImGui::GetScrollY() + (new_display_y - old_display_y);
...
void ImageCanvas::drawCheckerboard(ImDrawList& draw_list,
                                   ...
                                   ImVec2 max,
                                   float square_size) {
    const ImU32 light = IM_COL32(204, 204, 204, 255);
    const ImU32 dark = IM_COL32(136, 136, 136, 255);
    ...
    for (float y = min.y; y < max.y; y += square_size, ++row) {
        ...
        for (float x = min.x; x < max.x; x += square_size, ++column) {
            ...
            const ImVec2 cell_max{ std::min(x + square_size, max.x),
                                   std::min(y + square_size, max.y) };

            const bool is_light = ((row + column) & 1) == 0;

            draw_list.AddRectFilled(cell_min, cell_max, is_light ? light : dark);
        }
    }
}

ImageCanvasResult ImageCanvas::draw(const ImageCanvasDesc& desc) {
    ...
    if (desc.show_toolbar) {
        toolbar_height = ImGui::GetFrameHeightWithSpacing();

        Vec2f fit_area{ widget_size.x,
                        std::max(widget_size.y - toolbar_height, 1.0f) };

        drawToolbar(desc, fit_area);
    }

    const Vec2f child_size{ widget_size.x,
                            std::max(widget_size.y - toolbar_height, 1.0f) };

    const ImGuiWindowFlags child_flags = ImGuiWindowFlags_HorizontalScrollbar |
                                         ImGuiWindowFlags_AlwaysVerticalScrollbar;

    const bool child_visible = ImGui::BeginChild("##ImageCanvasScroll",
                                                 ToImVec2(child_size),
    ...
    Vec2f content_size = ToVec2f(ImGui::GetContentRegionAvail());
    ...
    const Vec2f displayed_size{
        math::max(desc.image_size_px.x * m_zoom, 1.0f),
        math::max(desc.image_size_px.y * m_zoom, 1.0f),
    };

    // Center the image when it is smaller than the viewport.
    const Vec2f padding{
        math::max((content_size.x - displayed_size.x) * 0.5f, 0.0f),
        math::max((content_size.y - displayed_size.y) * 0.5f, 0.0f),
    };
    ...
    const ImVec2 image_min_ss = ImGui::GetCursorScreenPos();

    const ImVec2 image_max_ss{ image_min_ss.x + displayed_size.x,
                               image_min_ss.y + displayed_size.y };

    ImDrawList* draw_list = ImGui::GetWindowDrawList();

    if (desc.show_checkerboard) {
        drawCheckerboard(*draw_list,
                         image_min_ss,
                         image_max_ss,
                         8.0f * m_zoom);
    }
    ...
    if (desc.texture != 0) {
        ImGui::Image(desc.texture, ToImVec2(displayed_size));
    } else {
        ImGui::InvisibleButton("##MissingImage", ToImVec2(displayed_size));
    }

    result.hovered = ImGui::IsItemHovered();
    result.left_clicked = result.hovered && ImGui::IsMouseClicked(ImGuiMouseButton_Left);

    result.left_double_clicked = result.hovered && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left);
    ...
    if (result.hovered) {
        ...
        result.mouse_image_px =
            Some(Vec2f{ (mouse_ss.x - image_min_ss.x) / m_zoom,
                        (mouse_ss.y - image_min_ss.y) / m_zoom });
    }

    applyMouseWheelZoom(desc,
                        image_min_ss,
