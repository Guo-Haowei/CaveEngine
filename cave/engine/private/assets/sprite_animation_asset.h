...
public:
    ...
    SpriteAnimationClip(std::vector<OldRect>&& p_frames, float p_length = 1.0f);

    bool IsLooping() const { return m_loop; }

    void SetFrames(std::vector<OldRect>&& frames);

    void SetAnimationLength(float p_length);

    float GetTotalDuration() const { return m_total_duration; }

    const std::vector<OldRect>& GetFrames() const { return m_frames; }
...
public:
    bool AddClip(std::string&& p_name, std::vector<OldRect>&& p_frames);
