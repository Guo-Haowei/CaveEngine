...
#define ERR_FAIL_V_MSG_INTERNAL(RET, MSG, EXTRA)                                                         \
    do {                                                                                                 \
        ::cave::ReportErrorImpl(__FUNCTION__, __FILE__, __LINE__, "Method/function failed." EXTRA, MSG); \
        return RET;                                                                                      \
    } while (0)

#define ERR_FAIL_COND_V_MSG_INTERNAL(EXPR, RET, MSG, EXTRA)                                                            \
    if (!!(EXPR)) [[unlikely]] {                                                                                       \
        ::cave::ReportErrorImpl(__FUNCTION__, __FILE__, __LINE__, "Condition \"" _STR(EXPR) "\" is true." EXTRA, MSG); \
        return RET;                                                                                                    \
    } else                                                                                                             \
        ((void)0)
...
#define ERR_FAIL_INDEX_V_MSG_INTERNAL(INDEX, BOUND, RET, MSG)                                                        \
    if (int64_t index_ = (int64_t)(INDEX), bound_ = (int64_t)(BOUND); index_ < 0 || index_ >= bound_) [[unlikely]] { \
        ::cave::ReportErrorIndexImpl(__FUNCTION__, __FILE__, __LINE__, "", index_, bound_, _STR(INDEX), _STR(BOUND), \
                                     MSG);                                                                           \
        return RET;                                                                                                  \
    } else                                                                                                           \
        ((void)0)
...
#define CRASH_NOW()                                                                                \
    do {                                                                                           \
        cave::ReportErrorImpl(__FUNCTION__, __FILE__, __LINE__, "FATAL: Method/function failed."); \
        GENERATE_TRAP();                                                                           \
    } while (0)

#define CRASH_NOW_MSG(MSG)                                                                              \
    do {                                                                                                \
        cave::ReportErrorImpl(__FUNCTION__, __FILE__, __LINE__, "FATAL: Method/function failed.", MSG); \
        GENERATE_TRAP();                                                                                \
    } while (0)

#define CRASH_COND(EXPR)                                                                                         \
    if (auto _cond = !!(EXPR); _cond) [[unlikely]] {                                                             \
        cave::ReportErrorImpl(__FUNCTION__, __FILE__, __LINE__, "FATAL: Condition \"" _STR(EXPR) "\" is true."); \
        GENERATE_TRAP();                                                                                         \
    } else                                                                                                       \
        (void)0
...
#define CRASH_COND_MSG(EXPR, MSG)                                                                                     \
    if (auto _cond = !!(EXPR); _cond) [[unlikely]] {                                                                  \
        cave::ReportErrorImpl(__FUNCTION__, __FILE__, __LINE__, "FATAL: Condition \"" _STR(EXPR) "\" is true.", MSG); \
        GENERATE_TRAP();                                                                                              \
    } else                                                                                                            \
        (void)0
...
#define DEV_ASSERT(EXPR)                                                                                      \
    if (auto _cond = !!(EXPR); !_cond) [[unlikely]] {                                                         \
        cave::ReportErrorImpl(__FUNCTION__, __FILE__, __LINE__, "FATAL: DEV_ASSERT failed (" _STR(EXPR) ")"); \
        GENERATE_TRAP();                                                                                      \
    } else                                                                                                    \
        ((void)0)
...
#define DEV_ASSERT_MSG(EXPR, MSG)                                                                                  \
    if (auto _cond = !!(EXPR); !_cond) [[unlikely]] {                                                              \
        cave::ReportErrorImpl(__FUNCTION__, __FILE__, __LINE__, "FATAL: DEV_ASSERT failed (" _STR(EXPR) ")", MSG); \
        GENERATE_TRAP();                                                                                           \
    } else                                                                                                         \
        ((void)0)
...
#define DEV_ASSERT_INDEX(INDEX, BOUND)                                                                               \
    if (int64_t _index = (int64_t)(INDEX), _bound = (int64_t)(BOUND); _index < 0 || _index >= _bound) [[unlikely]] { \
        cave::ReportErrorIndexImpl(__FUNCTION__, __FILE__, __LINE__, "FATAL: DEV_ASSERT_INDEX failed ", _index,      \
...
#define DEV_VERIFY_CHECK(EXPR) \
    ((EXPR) || (cave::ReportErrorImpl(__FUNCTION__, __FILE__, __LINE__, "FATAL: Condition \"" _STR(EXPR) "\" is false."), false))
