...
struct alignas(sizeof(T) * 4) Vector<T, 4> : VectorBase<T, 4> {
...
#if USING(MATH_ENABLE_SIMD_SSE)
    constexpr Vector(__m128 simd)
        : simd(simd) {
    }
#elif USING(MATH_ENABLE_SIMD_NEON)
    constexpr Vector(float16x4_t simd)
        : simd(simd) {
    }
#endif
    ...
    explicit constexpr Vector(U x, U y, U z, U w)
    ...
    explicit constexpr Vector(const Vector<T, 3>& vec, T w) noexcept
        : x(vec.x), y(vec.y), z(vec.z), w(w) {
    }

    explicit constexpr Vector(const Vector<T, 2>& vec1, const Vector<T, 2>& vec2) noexcept
        : x(vec1.x), y(vec1.y), z(vec2.x), w(vec2.y) {
    }

    explicit constexpr Vector(const Vector<T, 2>& vec, T z, T w) noexcept
        : x(vec.x), y(vec.y), z(z), w(w) {
    }

    template<int N, int A, int B, int C, int D>
    constexpr Vector(const Swizzle4<T, N, A, B, C, D>& swizzle)
